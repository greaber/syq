...
`--follow` opts the whole native command into resolving every symlink in these
directly supplied paths, regardless of link ownership. It applies uniformly to
`--cwd`, `rm --root`, source selectors, copy placement paths, and local control
file paths such as `--ignore-from`, `--mapping`, and `--results`. It does not
make SYQ follow symlinks discovered beneath a selected directory, nor paths
generated by a mapping manifest, scanner, or peer.
...
For an exact placement, the same distinction applies to the last path
component: `--as link` addresses and may replace the symlink itself, while
`--follow --as link` addresses its referent. The `new` and `existing`
preconditions are checked against whichever object that choice selects. An
`--into` target must resolve to a directory, so a symlink there is refused by
default and accepted only with `--follow`. Consequently, a dangling link is an
existing link without `--follow`; with `--follow`, its referent is missing and
a placement form that permits creation may create that referent. In particular,
if `live` points to `../releases/v3`, `--follow --into live` uses `v3` as the
container, while `--follow --as live` updates `v3` itself. Exact placement does
not transplant the spelling `live` into the referent's parent.
