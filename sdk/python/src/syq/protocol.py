...
def parse_mapping_line(line: bytes) -> MappingEntry:
    ...
    unknown = set(record) - {"src", "dst", "kind", "size", "mtime", "expected_digest", "metadata"}
    if unknown:
        raise SyqProtocolError(
    ...
    try:
        return MappingEntry(src.raw, dst.raw, kind, size, mtime, _expected_digest(record), _destination_metadata(record))
    except (TypeError, ValueError) as error:
