...
fn run_transfer(args: Args, progress: Arc<Progress>) -> Result<i32> {
    ...
    if opts.benchmark.is_some() {
        crate::output::diagnostic!(
            "syq: tuning: request-size={} bytes (ordinary, after pacing and receiver limits), streaming-block-size={} bytes, pipeline-depth={}, hash-block-size={} bytes, copy-path={}, block-reuse={}, batch-files={}, batch-bytes={}, split-min-size={}, bw-pacing={}",
    ...
    if scan_err.is_none() && !st.collision {
        ...
        // Hardlink aliases must all be validated before copying their payloads.
