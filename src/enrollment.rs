...
impl EnrollmentId {
    ...
    pub fn random() -> Self {
        let mut bytes = crate::tcp_records::random_bytes(ENROLLMENT_ID_BYTES);
...
impl EnrollmentPublicKey {
    /// Parse the public half of the dedicated enrollment key.
    ///
    /// Enrollment accepts only Ed25519. This key is generated by syq and is
    /// not the user's SSH/YubiKey signing key.
    pub fn parse(line: &str) -> Result<Self> {
        let mut words = line.split_ascii_whitespace();
        let algorithm = words.next().context("enrollment public key is empty")?;
        let blob = words
            .next()
            .context("enrollment public key has no key blob")?;
        if algorithm != ED25519_ALGORITHM {
            bail!("enrollment key must use {ED25519_ALGORITHM}");
        }
        let decoded = base64::engine::general_purpose::STANDARD
            .decode(blob)
            .context("enrollment public key is not valid base64")?;
...
fn validate_ed25519_blob(mut blob: &[u8]) -> Result<()> {
    if take_ssh_string(&mut blob)? != ED25519_ALGORITHM.as_bytes() {
        bail!("enrollment public key blob has the wrong algorithm");
    }
    if take_ssh_string(&mut blob)?.len() != ED25519_KEY_BYTES {
        bail!("enrollment public key has the wrong Ed25519 key length");
    }
    if !blob.is_empty() {
        bail!("enrollment public key blob has trailing data");
    }
    Ok(())
}
...
impl AuthorizedKeyEntry {
    pub fn new(id: EnrollmentId, receiver_path: &Path, key: &EnrollmentPublicKey) -> Result<Self> {
...
pub fn install_authorized_key(
    ...
    for line in original.split(|byte| *byte == b'\n') {
        ...
        key_collision |= line_contains_enrollment_key(line, &entry.key);
    }
    if exact > 1 {
    ...
    if key_collision {
        bail!("enrollment key is already authorized by another authorized_keys line");
    }
    if exact == 1 {
...
pub fn revoke_authorized_key(
    ...
    for line in original.split_inclusive(|byte| *byte == b'\n') {
        ...
        if without_newline == entry.line().as_bytes() {
            ...
            key_collision |= line_contains_enrollment_key(without_newline, &entry.key);
            updated.extend_from_slice(line);
        }
    }
    if found > 1 {
    ...
    if key_collision {
        bail!(
            "enrollment key is also authorized by another authorized_keys line; refusing revocation"
        );
    }
    Ok((
        if found == 0 {
...
fn line_contains_enrollment_key(line: &[u8], key: &EnrollmentPublicKey) -> bool {
    let mut words = line
...
mod tests {
    ...
    fn key() -> EnrollmentPublicKey {
        ...
        EnrollmentPublicKey::parse(&format!(
    ...
    #[test]
    fn enrollment_key_parser_checks_the_wire_blob() {
        let mut bad = Vec::new();
        ...
        let error = EnrollmentPublicKey::parse(&format!(
            "ssh-ed25519 {}",
    ...
    #[test]
    fn install_and_revoke_refuse_any_other_authorization_for_the_enrollment_key() {
        let enrollment_key = key();
        let entry =
            AuthorizedKeyEntry::new(id(), Path::new("/opt/syq/receiver"), &enrollment_key).unwrap();
        let unrestricted = format!(
            "{} {} administrator-key\n",
            enrollment_key.algorithm, enrollment_key.blob
        );
        ...
        let other = AuthorizedKeyEntry::new(
            other_id,
            Path::new("/opt/syq/other-receiver"),
            &enrollment_key,
        )
        .unwrap();
        assert!(
