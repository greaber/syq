...
fn tiny_copy_if_sees_original_source_and_renamed_destination() {
    ...
    let predicate = format!("src.name = 'original' and dst.path = 'renamed' and dst.exists and src.mode = 0o400 and src.inode = {} and src.nlink = 1 and src.ctime <= now and src.size > dst.size and src.s3_last_modified is null and dst.s3_last_modified is null", meta.ino());
