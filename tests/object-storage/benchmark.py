            ...
            for repeat in range(args.repeats):
                for tool in (['s5cmd', 'syq'] if repeat % 2 == 0 else ['syq', 's5cmd']):
                    ...
                    if tool == 's5cmd':
                        ...
                        base = [executables[tool], 'cp', '--no-progress', '--performance-tuning', f's3-max-concurrent-objects={args.workers},s3-max-concurrent-parts-per-object={args.concurrency},s3-part-size={args.part_size}M', *flags]
