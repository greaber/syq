...
fn bounded_persistent_ssh_tree_starts_one_worker_and_recovers_channel_refusal() {
    ...
    for i in 0..300 {
        write(&t.path(&format!("src/file-{i}")), b"bounded tree payload");
    }
    for explicit in [false, true] {
        let destination = if explicit { "fixed" } else { "auto" };
        fs::create_dir_all(t.path(destination)).unwrap();
        let mut command = Command::new(env!("CARGO_BIN_EXE_syq"));
        command
            .args(["cp", "--no-tcp", "--no-progress", "--stats", "--pscope"])
            .arg(&scope)
            .arg("--syq-path")
            .arg(env!("CARGO_BIN_EXE_syq"))
            .arg("--srcs-in")
            .arg(t.path("src"))
            .args(["--to", "fake", "--into-existing"])
            .arg(t.path(destination))
            .env("FAKE_REMOTE_HOME", t.path("remote-home"))
            .env("FAKE_RSH_LOG", t.path("rsh.log"))
            .env("PATH", format!("{}:/usr/bin:/bin", t.s("bin")))
        ...
        for i in 0..300 {
            assert_eq!(
                read(&t.path(&format!("{destination}/file-{i}"))),
                b"bounded tree payload"
            );
        }
    }
    let invocations = fs::read_to_string(t.path("rsh.log")).unwrap();
    assert!(
        invocations.lines().any(|line| line.starts_with("auto|")),
        "{invocations}"
    );
    assert!(
        invocations
