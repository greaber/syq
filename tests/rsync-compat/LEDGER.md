...
| sparse | `sparse` | pass | Compatible | invocation adaptation (sparse-syq-cli) | platform=linux; sparse filesystem | Keep all upstream byte/allocation assertions; omit --no-sparse because sparse mode is already off by default. Linux only: the upstream test skips on the macOS CI runner, whose scratch filesystem does not create its sparse source file. |
