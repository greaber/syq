...
| security | `receiver-rename-symlink-race` | pass | Compatible | subset adaptation (rename-destination-symlink-race) of upstream rename-fullpath-symlink-race | platform=linux; symlinks | Race a destination parent between a directory and an out-of-tree symlink during normal staged publication; the adaptation removes upstream's unsupported absolute --temp-dir trigger but preserves its flipper, positive control, and outside-write oracle. |
