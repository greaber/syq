...
|---|---|---|---|---|---|---|
| critical | data-loss | [rsync-359](https://github.com/RsyncProject/rsync/issues/359) — Self-copy with source removal destroyed the source | partial | A copy whose effective source and destination are the same inode or tree must never truncate or overwrite its only good copy. | local:`inplace_self_copy_preserves_source`; local:`file_onto_itself_is_allowed_noop`; local:`copy_onto_itself_among_sources_is_order_independent`; local:`self_copy_guard_does_not_reject_a_destination_outside_the_moved_source`; local:`self_copy_guard_rejects_a_destination_inside_the_moved_source` | SYQ's supported ordinary and --inplace self-copy paths are guarded using retained source and destination directory capabilities, including across operator-path renames. Rsync's destructive --remove-source-files mode is not implemented, so the exact historical command is not applicable yet. |
...
| critical | security | [rsync-security-parent-symlink-races](https://github.com/RsyncProject/rsync/blob/7c20b077c980036a19587701cec320cc88e42a4a/SECURITY.md) — Parent-component symlink races can escape transfer roots | covered | Peer-selected source and destination paths must not be redirected outside their transfer roots by raced parent symlinks. | compat:`sender-scan-dir-escape`; compat:`symlink-race-source`; compat:`symlink-race-dest`; compat:`symlink-race-relative-dest`; local:`destination_root_replacement_after_selection_cannot_redirect_worker`; local:`destination_file_write_refuses_descendant_symlink_swap`; local:`destination_prune_scan_refuses_descendant_symlink_swap`; local:`self_copy_guard_sees_through_symlinks`; local:`files_from_self_copy_through_symlinked_root_is_rejected` | Default source discovery and reads plus destination scanning, mutation, file transfer, and publication use retained capabilities and refuse raced descendant symlinks. Rsync-mode --insecure-links is the documented explicit source-authority expansion, not a pathname fallback within the confined policy. |
