...
| critical | security | [rsync-security-parent-symlink-races](https://github.com/RsyncProject/rsync/blob/7c20b077c980036a19587701cec320cc88e42a4a/SECURITY.md) — Parent-component symlink races can escape transfer roots | partial | Peer-selected source and destination paths must not be redirected outside their transfer roots by raced parent symlinks. | compat:`sender-scan-dir-escape`; compat:`symlink-race-source`; compat:`symlink-race-dest`; compat:`symlink-race-relative-dest`; local:`self_copy_guard_sees_through_symlinks`; local:`files_from_self_copy_through_symlinked_root_is_rejected` | Source-side confinement passes. The two root-only destination tests deliberately retain expected failures for attacker-owned symlinks in operator-named destination paths, so this remains partial and highly visible. |
