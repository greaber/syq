...
This is the native selection policy, not a complete hostile-namespace
...
operations do not all use descriptor-relative access. The default therefore
rejects links present during preflight; it does not prevent a concurrent
rename or link substitution from changing an identity after that check.
...
(see [Mappings](mappings.md)). For a direct remote-to-remote copy,
...
results schema does not represent deletions. `--max-delete` requires
...
closed on that receiver. Other comparison and selection controls, and
...
The rsync-shaped command surface is available under `syq rsync`:
...
The sections below document this compatibility surface unless they
...
| `--insecure-links` | Allow a receiving process to follow destination-path symlinks owned by other users (unsafe opt-out) |
...
signed policies and the options that fail closed under it, enrollment
lifecycle, and the escape hatches are documented in
...
links owned by an unprivileged user fail it. `--insecure-links` restores
follow-any-link behavior. The `changes` line separately accounts for regular files,
