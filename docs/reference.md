...
Add `--dry-run` to any copy. Add `-v` for individual changes:
...
`--inplace` saves room for that second file but exposes incomplete updates
to readers and after interruption. Use it only when you accept that tradeoff.
Do not run the same logical copy twice concurrently: the runs share partial
files. Abandoned partial files can be deleted when you no longer intend to
resume that copy.

## Check file contents

Syq normally skips files whose size and modification time match.
`--hash` compares contents too, and repairs differences:
...
`--root DIR` confines source selection beneath that directory. Use
`--min-size` and `--max-size` to select regular files by size.
